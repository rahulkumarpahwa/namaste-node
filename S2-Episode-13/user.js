...
userRouter.get("/feed", userAuth, async (req, res) => {
  try {
    const data = await User.find({}).select("-password -email -createdAt -updatedAt"); // removing the password and email from the feed.
...
userRouter.get("/connections", userAuth, async (req, res) => {
  try {
    ...
    const allConnections = await ConnectionRequestModel.find({
      ...
      status: "accepted",
    }).populate("toUserId fromUserId","firstName lastName photoUrl age gender about"); // populate is only possible after we have make the 'ref' reference in the connectionRequestSchema.;
...
userRouter.get("/requests/received", userAuth, async (req, res) => {
  try {
    ...
    }).populate("fromUserId", ["firstName", "lastName" , "photoUrl", "age", "gender", "about", "skills"]).populate("toUserId", ["firstName", "lastName" , "photoUrl", "age", "gender", "about", "skills"]); // populate is only possible after we have make the 'ref' reference in the connectionRequestSchema.
